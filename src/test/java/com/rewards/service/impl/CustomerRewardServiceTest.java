...
public class CustomerRewardServiceTest {
	...
	public void testDeleteCustomerTransactionDeleteFails() {
		...
		doThrow(new RuntimeException(Messages.TRANSACTION_NOT_FOUND)).when(transactionRepository).deleteById(anyLong());

		assertThrows(RuntimeException.class, () -> {
			customerRewardService.deleteCustomer(1L);
		});

		verify(transactionRepository, atLeastOnce()).deleteById(anyLong());
